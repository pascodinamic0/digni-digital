...
import {
  AI_EMPLOYEE_SETUP_PROMO_END_MS,
  isAiEmployeeSetupPromoActive,
} from "@/lib/ai-employee-setup-promo";
      ...
      >
        <div className="relative z-10 flex flex-col gap-8 md:flex-row md:items-center md:justify-between">
          <div className="mx-auto max-w-xl text-center md:mx-0 md:text-left">
            <p className="type-body-lg leading-relaxed text-foreground/85">
              {p.assessmentNote}
            </p>
            <p className="type-caption mt-4 text-success/90">
              {p.investmentNote}
            </p>
          </div>
      ...
      <div className="p-8 text-center md:p-10">
        ...
        <div className="mx-auto mb-8 grid max-w-md grid-cols-2 gap-4">
          <div className="rounded-lg border border-border/60 bg-background/80 px-4 py-4">
            <div className="type-caption mb-2 font-semibold uppercase tracking-wider text-muted">
              {p.setupLabel}
            </div>
            {promoActive ? (
              <>
                <div className="type-small text-muted line-through decoration-destructive/40">
                  {p.setupFee}
                </div>
                <div className="type-h4 font-display font-bold text-success">
                  {p.setupFeeWaivedDisplay}
                </div>
              </>
            ) : (
              <div className="type-h4 font-display font-bold text-text">
                {p.setupFee}
              </div>
            )}
          </div>
          <div className="rounded-lg border border-success/25 bg-success/10 px-4 py-4">
            ...
            <div className="type-h4 font-display font-bold leading-none text-success">
              {p.price}
              <span className="type-small font-normal text-muted">
                {p.period}
              </span>
            </div>
          </div>
        ...
        {promoActive ? (
          <p className="type-small mb-6 text-success/90 leading-snug">
            ...
            <span className="font-semibold tabular-nums" aria-live="polite">
              {days > 0 && `${days}d · `}
              {padCountdownUnit(hours)}h · {padCountdownUnit(minutes)}m ·{" "}
