...
export function learningPath(job: AiCareerJob): string {
 ...
 { label: 'Week 3 to 4, Sell a pilot', steps: job.learningSteps.length > 4 ? job.learningSteps.slice(4) : ['Package a fixed scope offer with price, turnaround, and revision policy.', 'Deliver for one real or realistic client; capture testimonial and before/after.'] },
 ]
 return `
...
export function portfolioProof(job: AiCareerJob, proofItems: string[]): string {
 ...
 <li>A 3 to 5 minute Loom explaining your decisions (builds trust faster than a PDF alone)</li>
