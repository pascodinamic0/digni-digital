/** Public URL path for the Digni brand mark (nav logo, schema.org logo). */
export const BRAND_LOGO_PATH = '/Icon%20Logo%20DD.png'

/** Shield handshake mark for software demo workspace chrome. */
export const BRAND_SHIELD_LOGO_PATH = '/brand/digni-shield-logo.png'

/** Public URL path for the Digni full wordmark used in site chrome. */
export const BRAND_WORDMARK_PATH = '/digni-digital-wordmark-transparent.png'

/** Founder headshot (`public/images/pascal-digny-headshot.png`) — about, blog byline, etc. */
export const AUTHOR_HEADSHOT_PATH = '/images/pascal-digny-headshot.png'

/** Next.js serves `app/icon.png` at this path; use for explicit links. */
export const FAVICON_PATH = '/icon.png'
